...
- The name of the hash algorithm, if set explicitly, is checked against a list of names as returned from `openssl list -digest-commands`. Also, setting the algorithm is now done via accessors, with an extra accessor giving access to the list of available names.
...
- Synching with the latest version of the official draft (June 2023), namely:
  ...
  - The maximum level of recursion in the "Hash N Degree Quads" has a default (currently 50) which can be set to a lower level by the user. 
